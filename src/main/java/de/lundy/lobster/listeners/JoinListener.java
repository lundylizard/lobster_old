...
public class JoinListener extends ListenerAdapter {
    ...
    public void onGuildJoin(@NotNull GuildJoinEvent event) {
        ...
        Lobster.LOGGER.info("Joined Discord Server {} ({}) -- {}", event.getGuild().getName(), event.getGuild().getMemberCount(), event.getGuild().getIdLong());

        // Create guild tables on join
        if (!settingsManager.serverInSettingsTable(event.getGuild().getIdLong())) {
            Lobster.LOGGER.info("{} is not in the database yet. Creating...", event.getGuild().getName());
